...
var getPlaces = async function (req: any, res: any) {
  await dal.initApiKeyDictionary();
...
var getFlights = async function (req: any, res: any) {
  ...
  if (req.body){
    ...
    await dal.initApiKeyDictionary();
    let allData = await dal.getFlights(serialized);
...
function getFlightOption(offer, dictionaries, index)
{
  ...
  offer.itineraries[index].segments.forEach(seg =>
  {
      ...
      let flight = CreateFlight(seg.number, new Date(seg.departure.at),
          new Date(seg.arrival.at), CreateCarrier(seg.carrierCode, carrierName),
          getFullDuration(seg.duration),
          CreatePlace(seg.departure.iataCode, locationNameDepature.cityCode), CreatePlace(seg.arrival.iataCode, locationNameArrival.cityCode));
